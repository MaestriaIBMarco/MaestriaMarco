...
\begin{figure}[ht]
    \begin{center}
        ...
        \caption{Distribución geográfica de la especie de tortuga \textit{Chelonoidis chilensis \LGM{podés aprovechar para comentar dónde están las tortugas en cuestión, para poner el estudio en perspectiva. Además. poner puntos al final de las oraciones en este y el resto de los captions.}}
...
\section{Metodologias \LGM{acento}}
...
\end{center}
    \caption[Unidad de navegación comercial i-gotU para monitorear individuos.] {Unidad de navegación i-gotU para monitorear individuos.\LGM{espacio}Pesa aproximadamente  21\,g, las posiciones del GPS son adquiridas cada 15 minutos y se almacenan en la memoría interna del dispositivo. }
    \label{fig:igotu}
\end{figure}
...
El uso de herramientas de redes complejas permitió comparar resultados provenientes de diferentes análisis sobre los datos de GPS de las tortugas. En este trabajo se utilizaron métricas sobre la topología de distintas redes generadas, todas calculadas a través de funciones implementadas en la librería networkX \LGM{en mayúscula por ser nombre propio} de python \LGM{en mayúscula por ser nombre propio} \cite{networkx}.
 
Una red compleja es un conjunto de elementos (nodos) conectados entre sí por enlaces (aristas) \LGM{esta es la definición de una red, puede ser simple. Las redes se consideran complejas cuándo tienen características que las distingen de modelos generadores sencillos como, por ejemplo, el modelo de Erdös-Rényi}. La topología \LGM{podrías mencionar a qué consideras tolología} de una red compleja \LGM{cualquiera, compleja o simple, en realidad} puede ser estudiada a través de distintas métricas. En este trabajo se utilizaron las siguientes métricas:
\begin{itemize}
    ...
    \item  \textbf{Modularidad}: es una medida de fuerza de división de la red en comunidades \LGM{podrías mencionar qué consideras como 'comunidades'}. Se calcula como:
    \begin{equation}
        Q = \sum_{c=1}^{n}
...
\LGM{sobre los últimos dos, sería bueno plantear porqué o para qué se los considera. En realidad esto es válido para todas las métricas. Si no, queda medio descolgado del resto creo yo.}    
\end{itemize}
